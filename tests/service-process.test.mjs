...
test('all five badges render readable text and detail retains actor/date and read-only controls',()=>{
 ...
 assert.match(html,/aria-labelledby="service-process-title"/);assert.match(html,/fieldset disabled/);assert.match(html,/Бат/);assert.match(html,/Захиалгын ажилтан/);assert.match(html,/Ирсэн|ирсэн/);assert.doesNotMatch(html,/<form/);
});
