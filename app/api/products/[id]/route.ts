...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (Object.keys(changes).length) await writeAuditLog({ db: getDb(), action: row.active ? "product.updated" : "product.disabled", entityType: "product", entityId: row.id, entityRef: row.name, details: changes });
